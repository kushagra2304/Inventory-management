  ...
  return (
    <div className="p-6 space-y-10 bg-gray-50 min-h-screen">
      ...
      {/* <div className="flex flex-col md:flex-row justify-between gap-6">
                ...
                return (
                  <div key={idx}>
                    <div className="flex justify-between mb-1">
                      <span className="text-sm font-medium text-red-700 underline cursor-pointer" onClick={() => handleProductClick(item)}>
                        {item.comp_code}
                      </span>
                      <span className="text-xs text-gray-600">{item.quantity} left</span>
                    </div>
                    <div className="w-full h-2 bg-red-100 rounded-full">
                      <div className={`h-2 ${barColor} rounded-full`} style={{ width: `${percent}%` }} />
                    </div>
                  </div>
                );
              })}
        ...
        <div className="w-full md:w-1/2 bg-white rounded-2xl p-6 shadow-md">
          <h2 className="text-2xl font-semibold text-blue-900 mb-4">Product Quantity Distribution</h2>
          {pieData.length > 0 ? (
